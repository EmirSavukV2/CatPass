...
export function SecretsTable({ selectedProject, selectedGroup }: SecretsTableProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [showPasswords, setShowPasswords] = useState<{ [key: string]: boolean }>({});
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingSecret, setEditingSecret] = useState<{ id: string; data: SecretData } | null>(null);
  ...
  const { 
    ...
    updateSecret, 
    deleteSecret 
  } = useSecrets(selectedProject || undefined, selectedGroup || undefined);
  ...
  const handleEditSecret = async (secretId: string) => {
    ...
    if (decryptedData) {
      setEditingSecret({ id: secretId, data: decryptedData });
      setIsModalOpen(true);
    }
  };

  const handleSaveSecret = async (secretData: SecretData) => {
    try {
      if (editingSecret) {
        ...
        if (selectedGroup) {
          ...
          projectId = selectedProject || 'default'; // Use selected project or default
          ownerType = 'group';
        ...
        await createSecret(secretData, projectId, ownerType, ownerId);
